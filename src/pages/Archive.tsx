...
const Archive = () => {
  ...
  return <div className="min-h-screen bg-black text-white font-inter">
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="border border-gray-700 rounded-lg p-8 bg-transparent">
          <h2 className="text-2xl font-bold text-white mb-6">All Posts</h2>
          <ul className="space-y-4">
            {blogPosts.map(post => <li key={post.id} className="flex items-center">
                <span className="w-2 h-2 bg-white rounded-full mr-4 flex-shrink-0"></span>
                <div className="flex-1">
                  <Link to={post.url} className="text-white hover:text-gray-300 transition-colors font-medium">
                    {post.title}
                  </Link>
